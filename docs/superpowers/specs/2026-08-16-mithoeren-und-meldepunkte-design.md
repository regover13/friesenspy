...
| Farbe | Magenta `#e07be0` mit dunklem Saum (`drop-shadow` ohne Versatz, CSS-Klasse wie `.platzrunde`) | Kartenüblich für Meldepunkte, und im Haus noch frei: Blau `#2d9cdb` ist Klickbarem vorbehalten, Orange `#ff7a18` sind die Platzrunden (bis 10.09.2026 Hellblau `#8ab4d8`), Grau die FSE-Landeflächen. Der Saum trägt das Symbol auf Satellit **und** auf hellem CARTO |
