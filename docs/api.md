...
> **Kalender-Fracht:** Ein Termin mit dem `friesenkutter`-Marker kann direkt in der Beschreibung eine Fracht-Zeile enthalten: `Fracht: 1000 Krabbenbrötchen, 500 Friesentee`. Beim Sync wird sie **einmalig** (nur beim erstmaligen Anlegen) gegen den Frachtart-Katalog abgeglichen und ins Manifest übernommen; ein später im Admin gepflegtes Manifest bleibt bei erneutem Sync unverändert. **Fracht je Startplatz (v8.13.0/#15 Sub-Projekt B):** ein optionaler Startplatz-ICAO direkt am Marker bindet die Fracht — `Fracht EDDW: 500 Äpfel, 200 Nüsse` und `Fracht EDWG: 300 Birnen` (je eigene Zeile, mehrere Marker werden alle gelesen). `Fracht:` ohne ICAO bleibt geteilt (von jedem Startplatz ladbar). Die Marker-ICAOs wandern **nicht** in die Route; ein streckenfremder Startplatz (oder das Ziel) fällt beim Speichern auf „geteilt" zurück.
