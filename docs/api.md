...
**Response** — Array je Event: `id, name, route, destination, dtstart, dtend, source` (`calendar`|`manual`), `radius_km` (Legacy-Feld, seit GPS-only Phase 2/#23 wirkungslos — die Platz-Zuordnung nutzt überall den festen globalen 4-km-Radius; über die Admin-Endpoints nicht mehr setzbar, i. d. R. `null`), `total_kg` (= Σ tatsächlich am Ziel gelieferter Fracht = Σ Ziel-Stapel, „durchgängig Netto" — Überschuss ohne Manifest-Platz zählt nicht; ein Event **ohne** Manifest liefert 0), `target_kg` (= Σ Manifest oder `null`), `progress_pct` (oder `null`), `flight_count`, `loaded_count`, `cargo` (`[{name, target_kg, delivered_kg, reserved_kg, lost_kg, pct, per_flight_max_kg, departure}]`; `lost_kg` (v8.20.0/#7/#8) = unwiederbringlich verlorene (`stolen`/`sunk`) Menge dieser Frachtart, **netto** (nur was real an Bord war, nicht das volle Ziel) — verlorene Ware wird dauerhaft aus dem ladbaren Rest genommen (`verfügbar = target − delivered − lost − reserved`), macht ein Event ggf. unvollendbar und zeigt, warum es <100 % bleibt; `returned` zählt hier NICHT (Ware kam heil zurück); `per_flight_max_kg` = Obergrenze pro Flug oder `null`; `departure` = gebundener Startplatz — **genau EIN ICAO ≠ Ziel, Pflicht** (Stapel-Modell, Entscheidung 6: eine Zeile = ein Stapel = ein Ort; keine CSV-Liste und kein „geteilter Topf"/`null` mehr). Nur Flüge von diesem Startplatz laden die Frachtart, das Frontend gruppiert die Balken danach. Es gibt kein separates `Strecke`-Feld: `route` wird aus den Startplätzen der Fracht + Ziel abgeleitet).

## GET /api/transport/event/{id}

Voller Live-Zustand eines Events: obige Felder plus `flights` (chronologisch, **neueste zuerst**): `{dep_time, cid, callsign, name, aircraft, dep, arr, tonnage_kg, onboard_kg, loaded, cargo_lines, cargo_name, in_air, airborne, reserved_kg, onboard_reserved_kg, flight_key, distance_nm, block_min, quip}` — Verlust-/Rückbringer-Zeilen zusätzlich `loss_kind`, `lost_kg`. Beladene (gelieferte) Flüge tragen `loaded: true` + `cargo_name` (die Frachtart, in die ihr Anteil überwiegend floss); `tonnage_kg` ist die tatsächlich am Ziel gutgeschriebene Menge (Netto — Überschuss ohne Manifest-Platz zählt nicht), `onboard_kg` = `tonnage_kg` (für die „belegt / an Bord"-Anzeige). Rückflüge/Leerbeine `loaded: false`, `tonnage_kg: 0`, `cargo_name: null`. `in_air: true` markiert einen noch offenen (verbundenen) Flug **mit Ware an Bord** (im Stapel-Modell IST die Bordladung die Reservierung): `reserved_kg` = Σ Bordladung, `onboard_reserved_kg` = volle Musterzuladung (Kapazität). `airborne` trennt „wirklich abgehoben" von „am Boden geparkt": `false`, solange der Pilot am Boden steht (`position` ist ein ICAO — Anzeige „lädt/steht in <Platz> 🅿️"; `dep` = dieser Standort aus dem GPS-Leg-Modell), `true` erst in der Luft (`position == None`, „unterwegs ✈️"). Geladene Ware bleibt über Zwischenlandungen an Bord. `loss_kind` ∈ `returned` (mit Ware an einem **Ladeplatz** ausgeloggt — kein Verlust) \| `stolen` (mit Ware an einem **fremden Platz** ausgeloggt) \| `sunk` (mit Ware **in der Luft** ausgeloggt) \| `null` (kein Verlust); `lost_kg` > 0 nur bei `stolen`/`sunk` (bei `returned` = 0, Ware kam heil zurück).

Zusätzliche Top-Level-Felder: `participants` (`[{cid, name, callsign, aircraft, flights, delivered_kg, reserved_kg, lost_kg, status, visible, place, last_ground, cargo_lines}]`; `status` ∈ `flying` (Ware an Bord, in der Luft) \| `loaded` (Ware an Bord, am Boden) \| `dabei` (leer in der Luft — macht noch mit) \| `loading` (leer, steht am Ladeplatz) \| `standing` (leer, steht am Ziel oder fremden Platz) \| `done` (Default ohne Feed-Zeile) — **`arrived`/`returning` entfallen ersatzlos**; die Live-Anzeige leitet das Frontend aus Ort × Ladung ab. `reserved_kg` = Σ Bordladung dieses Piloten (die Bordladung IST die Reservierung), `visible` = letzter Bodenkontakt an einem teilnehmenden Platz ODER Ladung > 0 (Entscheidung 14), `place` = ICAO, an dem er steht (`null` = unterwegs), `last_ground` = letzter Landeplatz, `cargo_lines` = Bordladung-Aufschlüsselung; `callsign` = zuletzt gesehenes Rufzeichen — der Kutter-Block im Live-Tab zeigt Callsigns statt Namen), `reserved_total_kg` (Σ aller Flieger-Stapel = Σ Bordladung), `lost_total_kg` (Σ STOLEN + SUNK), `losses` (Teilmenge von `flights` mit gesetztem `loss_kind`). `cargo_lines` (Bordladung, Co-Load-Aufschlüsselung) tragen gelieferte Flüge, Verlust-/Rückbringer-Zeilen UND unterwegs befindliche (`in_air`) Flüge. Die Event-Listen-Ausgabe (`GET /api/transport/events`) enthält zusätzlich `reserved_total_kg` + `lost_total_kg` je Event.

> **Stapel-Modell (Ladung ist ein Bestand mit einem Ort).** Die Regeln stehen vollständig in
> `app/transport_stacks.py` (Zustandsmaschine `derive_stacks`), die Herleitung in der Spec
> `docs/superpowers/specs/2026-07-15-kutter-stapel-modell-design.md`. Das Manifest liegt als
> „Stapel" an seinen Ladeplätzen; der Flieger trägt selbst einen Stapel mit einer Position.
> **Erhaltungssatz:** Σ Stapel + Σ Bordladung == Σ Manifest — Ware entsteht nicht und verschwindet
> nicht (Arithmetik, keine Zusicherung).

> **Reservierung = Bordladung.** Es gibt keinen getrennten Reservierungs-Topf mehr: wer am Boden an
> einem Ladeplatz steht, nimmt Ware **vom Stapel** — das IST die Reservierung. `reserved_kg` = Σ
> Bordladung, `reserved_total_kg` = Σ aller Flieger-Stapel. Was geladen ist, bleibt über
> Zwischenlandungen an Bord.

> **Lieferung ohne Latch.** Wer am `destination` **landet**, liefert **sofort** den ganzen
> Flieger-Stapel in den Ziel-Stapel — kein Disconnect nötig, kein Live-Ankunfts-Latch
> (`transport_live_arrivals`/`check_live_arrival` werden nicht mehr gelesen oder geschrieben).

> **Verluste = Ort beim Logout.** Loggt ein Pilot mit Ware an Bord aus (auch bei ungewolltem
> Disconnect), wandert die Bordladung dorthin, wo er gerade ist — an einem **Ladeplatz** →
> `returned` (kein Verlust), an einem **fremden Platz** → `stolen`, **in der Luft** → `sunk`.
> Es gibt keine separate Erkennung mehr (`detect_transport_losses`/`transport_cargo_losses`
> entfallen — die Tabelle bleibt für Altdaten, wird aber nicht mehr geschrieben/gelesen). Verluste
> kommen direkt aus dem Fortschritt: `losses[]`, `lost_total_kg`, `cargo[].lost_kg`.
...
**Wiederaufruf (seit 15.19.9, #48):** Schlägt eine der drei Prüfungen fehl, der Aufrufer hat aber eine gültige `fs_user`-Sitzung, antwortet der Rückruf mit `303` zum Ziel – `/panel` im Kniebrett (User-Agent `CoherentGT`), sonst `/`. Es wird dabei **keine** neue Sitzung ausgestellt. Ohne Sitzung bleibt es bei `400`/`401`, aber als lesbare HTML-Seite mit einem Knopf „Neu anmelden“ statt JSON. (Einen Knopf „Zurück zum Kniebrett“ gibt es bewusst nicht: Ohne Sitzung leitet das Gate `/panel` ohnehin zur Anmeldung.) Anlass: Coherent GT rief einen längst verbrauchten Rückruf erneut auf, und die JSON-Fehlerzeile blieb als schwarzes Tablet stehen.
...
Seit v9.5.0/#78 trägt jeder Eintrag optional einen `reason` (Grund). Bestandseinträge wurden
einmalig **aus den Daten** beschriftet, nicht aus einer gepflegten Code-Liste
(`migrate_custom_airport_reasons`, idempotent, `WHERE reason IS NULL` — ein vom Admin
gepflegter Text überlebt jeden `init_db`-Lauf): Code nicht in `airportsdata` → Ergänzung;
Koordinate weicht > 1 km ab → `airportsdata` steht am falschen Ort; Koordinate praktisch
unverändert → reiner Radius-Override. Die Regel ist damit auch für künftige Einträge korrekt,
die beim Schreiben der Migration noch niemand kannte.
...
{
  "airports": [
    {"icao": "ZZSALZ", "name": "Segelfluggelände Salzwedel/Klein Gartz", "lat": 52.828, "lon": 11.316, "elevation_ft": 112.0, "radius_km": null, "reason": "Fehlt in airportsdata", "updated_at": "2026-07-05T12:00:00Z"}
  ]
}
```
...
```json
{"icao": "zzsalz", "name": "Segelfluggelände Salzwedel/Klein Gartz", "lat": 52.828, "lon": 11.316, "elevation_ft": 112, "radius_km": null, "reason": "Fehlt in airportsdata"}
```
...
- `reason` (optional, Text, v9.5.0/#78) — **warum** dieser Eintrag existiert. Reine Dokumentation, **nie Pflicht** und ohne Wirkung auf die Erkennung: der Eintrag selbst ist die Funktion, ein fehlender Grund darf nichts blockieren. Freitext ohne Enum/Validierung — das Admin-UI bietet per `<datalist>` die bereits vergebenen Gründe als Autocomplete an (abgeleitet aus `GET /api/admin/airports`, **kein eigener Endpoint**), sodass neue Gründe durch Benutzung entstehen und alte konsistent wiederverwendet werden. Die drei eingeführten Standard-Gründe: `Fehlt in airportsdata` (Ergänzung), `airportsdata-Koordinate falsch` (Override, Fund: EBUL/EBKT/EBBR/ELLX/EBAW), `Abhebepunkt außerhalb Standardradius` (reiner Radius-Override, Fund: EHAM).
...
| `DELETE /api/admin/bruegge/soll/{id}` | Anforderung zurücknehmen. **`?gruppe=true`** nimmt eine ganze Matrix zurück: `{id}` ist dann ihr Präfix, weg sind genau die `<präfix>-<reihe>-<spalte>` (ein Einzelobjekt `<präfix>-x` bleibt) |
...
**Boden oder Höhe — es bleibt bei `auf_boden` (`OnGround=1`), wie bei den Booten.** Der Simulator setzt das
Objekt selbst auf den Boden, und die Höhe muss niemand kennen (12.09.2026 gemessen, vier Gattungen ohne jede
Höhenangabe; bis 10 km Entfernung). **Es gibt keine gerechnete Höhe und keine Sonde:** Die Sonde ist am 12.09.2026 auf
Nutzerentscheidung gestrichen (*„Streiche den Sondenumweg! Dass du nicht ständig wieder damit kommst."*), und die
