...
| `aircraft_payloads` | Zuladung je Flugzeugtyp (Admin-editierbar): `mtow_kg`/`empty_kg`/`fuel_kg` (Tankinhalt getrennt) → `payload_kg = max(0, mtow−empty−fuel)`, direkt überschreibbar; `source` ∈ `manual` \| `llm` \| `default`; `make_model` (Klartext von Claude) |
