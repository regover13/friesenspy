...
Pflichtfelder: `SECRET_KEY`. Optional: `STATSIM_API_KEY` für historische Flugdaten, `OPENAIP_API_KEY` für OpenAIP-Overlay im Frontend, `CARTO_API_KEY` für die Kartenebenen Light/Dark, `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_CONTACT_EMAIL` für Web Push Notifications, `LOG_LEVEL` (Default `INFO`).
...
| `calendar_events` | FriesenFlieger Google-Kalender (alle 6h synchronisiert, UID als Primary Key); `route` (CSV aller ICAOs) + `is_bummel` (Flag) für die FriesenBummel-Erkennung; `delete_stale_calendar_events` räumt bei jedem Sync Termine weg, die im Google-Kalender gelöscht wurden (Mark-and-Sweep über die im Lauf gelieferten UIDs, begrenzt auf das Sync-Fenster ±365/90 Tage) — `bummel_races`/`transport_events` bleiben davon unberührt. **Seit #19 (05.09.2026) wird nichts mehr am Stichwort verworfen:** Auch Termine mit „kutter" im Text landen hier und werden angezeigt; ob ein Termin zu einem Event-Objekt gehört, steht in dessen `calendar_uid` und wird im Admin von Hand ausgesprochen. Ein Termin, an dem ein Objekt hängt, wird von `get_calendar_events` nicht mehr geliefert und löst keine generische Erinnerung aus |
| `bummel_races` | Persistente Bummel-Rennen (vom Poller beim Kalender-Sync oder Admin manuell angelegt); `manual_fields` (CSV, #19) = die im Admin von Hand gesetzten Felder, die der Kalender-Sync nicht mehr überschreibt; `calendar_uid` = der Termin, der dazu gehört (im Admin wählbar); `revealed_at` steuert die Fairness-Verdeckung — `NULL` = noch verborgen, Zeitstempel = enthüllt (Latch); `started_at` = Start-Latch (erster Pilot mit Blockzeit an einem Streckenflugplatz, `NULL` = noch kein Start); `push_enabled` steuert Push-Benachrichtigungen je Rennen (1 = an, 0 = aus); `source` ∈ `calendar` | `manual`. `badge_name` = Kurzname für das Badge-PNG (NULL/leer = der Event-Name); der volle Name sprengt die runde Grafik. Bewusst **nicht** in `manual_fields`: der Kalender kennt das Feld nicht, eine Marke hätte dort einen Rückholknopf ohne Ziel |
...
| `transport_events` | Persistente FriesenKutter-Transport-Events. **Seit 20.07.2026 nur noch Admin/manuell** (`source = manual`) — ein Kalendertermin legt keinen Kutter an, er kann kein Frachtmanifest tragen; der `source = calendar`-Pfad (`upsert_calendar_transport_event`) bleibt für eine spätere forum-basierte Lösung im Code, wird aber von `_sync_calendar` nicht mehr gerufen. `calendar_uid` ist seit #19 im Admin setzbar: gesetzt = dieser Kutter IST der Kalendertermin (der erscheint dann nicht mehr separat), leer = beide stehen nebeneinander. `manual_fields` (CSV) hält fest, welche Felder ein Mensch geändert hat — der Kalender-Sync lässt genau diese stehen. `route` (ICAO-CSV), `destination` (Ziel-ICAO — nur Flüge dorthin laden Fracht), Latches `started_at`/`goal_reached_at`/`summarized_at` für die Pushs; `push_enabled` steuert Push-Benachrichtigungen je Event (1 = an, 0 = aus, analog `bummel_races.push_enabled`); `source` ∈ `calendar` \| `manual`. `badge_name` = Kurzname für das Badge-PNG (NULL/leer = der Event-Name), und wie bei `bummel_races` bewusst nicht in `manual_fields` |
...
Serverseitiges Badge-Rendering mit **Pillow** für Forensignaturen nach einem FriesenBummel bzw. FriesenKutter-Event. Alle Badges sind **rund (256 px)** mit transparenten Rändern und nutzen die FriesenFlieger-Markenhintergründe aus `app/static/badge/` (`winner_bg.png` / `medal_bg.png` — Flugzeug, ostfriesische Inselkette, Vereinsfarben aus dem Repaint-Kit). Der Text wird zentriert in die ruhigen Zonen gelegt, strikt in der FF-Palette (Navy `#191D53`, Hellblau `#8FBFF1`, Rot `#8A1B1B`, Orange `#D75F28`).
...
**Merker liegen auf dem Server (ab v13.6.3).** `_prefLies(key)` / `_prefSchreib(key, wert)` sind die einzige Ablage für alle Karten-Merker. Führende Quelle ist `GET/PUT /api/prefs?kontext=panel|web`, Tabelle `panel_prefs` (Schlüssel: CID + Kontext). Cookie `fs_karte` und `localStorage` bleiben als **lokaler Zwischenspeicher** bestehen — sie tragen die Website, sie tragen ohne Anmeldung und sie überbrücken den Moment zwischen Seitenaufbau und Serverantwort. Die neun Schlüssel behalten ihre Namen (`friesenspy_layer`, `friesenspy_aip`, `friesenspy_platzrunden`, `friesenspy_fse_plaetze`, `friesenspy_fse_zonen`, `friesenspy_verkehr`, `fs_verkehr_schilder`, `friesenspy_trackup`, `friesenspy_movingmap`) und ihre Vorgaben.

*Warum nicht im Browser — zwei gescheiterte Anläufe:* Im MSFS-Kniebrett hält **kein** Browser-Speicher über einen Sim-Neustart. Gemessen am 16.08.2026 über einen Neustart hinweg (`speicher`-Block im Panel-Bericht): `localStorage` fällt von 8 Schlüsseln auf 0, das Merker-Cookie ist fort (`merkerDa: false`), obwohl `schreibbar: true` bleibt — innerhalb der Sitzung funktioniert alles. v13.6.0 legte die Merker deshalb erfolglos in `localStorage`-Ersatz, v13.6.2 in ein Cookie mit `SameSite=None`. Beides war vergeblich, und beides war vermeidbar: `panel_devices` in `database.py` und die EFB-Shell halten seit dem 13.08. fest, dass Coherent GT Cookies nur im Speicher hält.

*Was dort überlebt:* MSFS' eigene, plattenpersistente Ablage (`SetStoredData`, gekapselt als `DataStore` in `msfs-panel/…/FriesenSpy.tsx`) — derselbe Weg, den Avionik-Erweiterungen wie das GTN 750 gehen. Dort liegt die **Geräte-ID**; über `/auth/device` verschafft sie dem Panel bei jedem Start ein frisches Sitzungs-Cookie. An dieser Kennung hängen die Merker. Deshalb ist die Reihenfolge im Frontend zwingend: `initLiveMap` wartet mit `await _prefsPromise`, **bevor** die Karte gebaut wird — Basisebene, Ebenen-Haken und Ausschnitt werden gleich beim Aufbau gelesen.
...
- **Im Kniebrett zeigt das Vollbild nur die Karte** (seit 02.09.2026): `toggleMapFullscreen` setzt über `_vollbildKlassePflegen()` die Klasse `karte-vollbild` am `<html>` — nicht am Karten-Wrapper, weil die Zurück-/Tab-Leiste bewusst außerhalb von `#app` am `</body>` hängt und über die Karte gar nicht ansprechbar wäre. Die Leiste klebte am oberen Rand und verdeckte alles, was die Karte dort anbietet, zuletzt die Eingabezeile der ICAO-Suche. Der Ausgang wandert dafür zurück in die Karte (`.map-fullscreen-btn`, unten links, `z-index: 1200` über der Herkunftsangabe) und heißt in beiden Zuständen schlicht „Vollbild“ — nur das Zeichen wechselt. Die Windanzeige bleibt fest oben links über den Zoomknöpfen, im Vollbild wie außerhalb.
- **Der Zurück-Knopf der Leiste ist am 02.09.2026 ersatzlos entfallen** (`panel-back-btn`, `panelGoBack`, `_panelCanGoBack`, `_updatePanelBackBtn` und das Fenster-Verzeichnis `_PANEL_MODALS`; `_initPanelBackButton` heißt jetzt `_initPanelTopbar`). Er entstand am 13.08.2026 als Notausgang: Wer ein Fenster öffnete, war auf dessen eigenen Schließen-Knopf angewiesen, und wenn dem Font das Glyph fehlte, saß man fest. **Beide Gründe sind erledigt** — alle sechs Fenster schließen über `#icon-x` aus dem SVG-Sprite (die Bauform, die im Kniebrett nachweislich rendert), und das Vollbild hat seinen eigenen Ausgang. Damit entfällt auch die frühere Sonderregel, die die Leiste bei offenem Fenster zurückholte: `_vollbildKlassePflegen` prüft nur noch den Vollbildzustand. **Die Bedingung, unter der das richtig bleibt**, steht als Test fest (`test_der_zurueck_knopf_ist_ersatzlos_entfallen`): Verliert auch nur ein Fenster seinen eigenen Ausgang, ist der Knopf wieder nötig.
- **Die vier Tabs teilen sich die Breite gleichmäßig** (`flex: 1 1 0` von der Grundlage null, nicht nach Textlänge). Die Regel hing vorher am ausgeblendeten Zurück-Knopf und galt deshalb nur, solange es nichts zu verlassen gab.
...
**Zuletzt betrachteter Ausschnitt (v13.6.0):** `_ausschnittStart()` / `_ausschnittBeobachten(map)`, Schlüssel `friesenspy_ausschnitt` im Format `"lat,lon,zoom"`. Nur die **Live-Karte**. Gespeichert wird gebündelt 2 s nach der letzten Bewegung (`moveend`/`zoomend` feuern beim Schieben in Serie, jeder Schreibvorgang setzt das ganze Cookie neu) und **nur bei Bewegungen des Nutzers**: `_naviSelbstBewegt`/`_naviZoomt` schließen die Nachführung aus, sonst schriebe die Moving Map im Sekundentakt und der gemerkte Ort wäre das Ende des letzten Fluges statt der zuletzt selbst betrachtete Ausschnitt. Der gelesene Wert wird **streng geprüft** (drei Felder, endliche Zahlen, Lat/Lon im Wertebereich, Zoom auf `[6, 19]` gezogen) — ein `NaN` als Mittelpunkt lässt Leaflet beim ersten Zeichnen werfen, und der gesamte Karten-Tab bliebe leer. Ohne gemerkten Wert startet die Karte wie bisher auf EDWG (`_KARTE_MITTE`/`_KARTE_ZOOM`).
...
- **Beide Quellen werden zusammengeführt** (`_verkehrZusammenfuehren`, ab v13.2.0) — nicht gegeneinander getauscht. Die erste Fassung ließ immer nur eine gelten: Sobald der Sim lieferte, brach `_verkehrAbrufen` ab. Im Flug fiel das sofort auf (15.08.2026): Beim vPilot-Connect verschwanden Flugzeuge, die VATSIM kannte und der Sim nicht — vPilot spawnt nicht jede Maschine (Model Matching, Sichtbarkeitsgrenzen) — **und mit ihnen alle Rufzeichen**, denn der Sim liefert `name`/`plane_model_icao` leer.
  - **Gepaart**: Position, Kurs und Geschwindigkeit vom **Sim**, Rufzeichen/Muster/Flugplan von **VATSIM**. Schlüssel ist das Callsign — reißt die Zuordnung ab und kommt wieder, bleibt es derselbe Marker.
  - **Nur Sim**: Schlüssel `sim:<uId>`, ohne Rufzeichen. **Nur VATSIM**: wie bisher.
  - **Jeder Eintrag trägt seinen eigenen Messzeitpunkt** (`_ts`) und Schlüssel (`_key`). Das muss am Eintrag hängen, seit beide Quellen in einer Liste stehen: Eine Sim-Meldung gilt für diese Sekunde, eine VATSIM-Meldung ist rund 29 s alt — ein gemeinsamer Wert würde die eine Hälfte falsch fortrechnen.
...
**Wie die Zuordnung entsteht (v13.5.x).** Spec: `docs/superpowers/specs/2026-08-16-verkehr-zuordnung-design.md`. Die erste Fassung suchte sie in **jedem Takt neu** und kippte an Grenzfällen — zwei Schilder an einem Airliner, ein Label das im Sekundentakt trennt. Zu jedem Schwellwert gibt es einen Grenzfall; der Fehler war, die Frage überhaupt jede Sekunde neu zu stellen.

- **Einmal erkennen, dann merken** (`_paarungen`, Schlüssel `uId`). Dass einmal reicht, liegt am Ablauf eines Fluges: Piloten stehen nach dem Einloggen minutenlang, und **im Stand ist die Latenz gegenstandslos** — eine 29 s alte Position eines stehenden Flugzeugs ist exakt richtig. Dort entsteht der Großteil aller Zuordnungen, auf Meter genau; bis zur Platzrunde stehen sie längst.
- **Nur für die Sitzung.** Gemessen: Die `uId` überlebt keinen Neustart (zwei Sitzungen, 14 und 20 Kennungen, **null** Übereinstimmungen; 169 Mio gegen 102 Mio, innerhalb einer Sitzung in 16384er-Schritten — ein Handle-Muster). Dauerhaft gespeichert hinge sie irgendwann ein falsches Rufzeichen an ein fremdes Flugzeug.
- **Eindeutig heißt Vorsprung, nicht „einer im Umkreis"** (`_PAARUNG_VORSPRUNG = 0.5`). Der beste Kandidat muss halb so weit weg sein wie der zweitbeste. Zwei Fehlversuche zeigten, dass eine Schranke das nicht leisten kann: 400 m fasste auf dem Vorfeld mehrere (nichts eindeutig), 150 m keinen mehr. Jede Zahl macht anderswo dasselbe Problem — ein Verhältnis nicht. **Wirkung im Flug: 3 → 16 Zuordnungen.**
- **Ausschluss als zweites Kriterium**, erst **nach** der Nähe: Bleibt beidseitig genau einer übrig, gehören sie zusammen. Trägt den Fall „der fünfte loggt sich in der Platzrunde ein" — die anderen vier sind bekannt. Vermischt man beides, nimmt eine schwache Nähe-Zuordnung einen Kandidaten weg, den der Ausschluss sicher zugeordnet hätte.
- **Schranken werden gerechnet, nicht gesetzt** (`_paarungMaxM`, `_paarungMaxFt`): `GS × 29 s × Faktor`, Höhe analog über die geglättete Sinkrate. Eine C172 darf 3 km abweichen, ein Airliner 13, eine Hornet im Überschall 22 — eine feste Zahl wäre für das eine zu weit und für das andere zu eng. Ein sinkender Airliner mit 1600 ft Differenz ist damit **erwartungsgemäß** statt ein Grenzfall.
- **Gelöst wird erst nach vier Verstößen in Folge** (`_PAARUNG_LOESEN_TAKTE`) und mit größerem Faktor als beim Zuordnen: Nach dem Merken kann Flackern nur noch beim Lösen entstehen.
- **Verdecken statt doppelt zeichnen** (Schritt 3b): Bleibt ein Sim-Flugzeug ohne Zuordnung, wird der nächstgelegene VATSIM-Marker **nicht gezeichnet** — nicht zugeordnet, nur verdeckt. Das behauptet nichts über seine Identität, sondern nur, dass er wahrscheinlich eines der Sim-Flugzeuge ist. Einer je unzugeordnetem Sim-Eintrag, damit die **Anzahl** stimmt. Ohne das gewann optisch der VATSIM-Marker mit Rufzeichen — und der hängt im 15-s-Takt, also genau die Umkehrung der Vorgabe „Position schlägt Identität".
- **Friesen** (`_friesenAlsKandidaten`) kommen in die Kandidatenliste, werden aber nicht als Fremdverkehr gezeichnet (`v._friese`). `/api/traffic` filtert sie heraus, der Sim kennt sie aber — ihre Sim-Meldung fand nie einen Partner und wurde als graues Symbol **neben** dem blauen gezeichnet: am FriesenFlieger-Freitag jeder Friese doppelt. Ist ein Friese zugeordnet, bekommt **sein** Marker die Sim-Werte (`_positionsRoh` für Position/Kurs, `_friesenSimWerte` fürs Schild) und `_markerGehoertDemSim` schützt ihn vor dem VATSIM-Zulauf.
- **Rohwerte auch bei geänderter Geschwindigkeit fortschreiben** (`alt.gs !== neueGs`). Die frühere Bedingung fragte nur nach der Position — die ändert sich beim Stehen nicht mehr, also blieb der alte VATSIM-Eintrag mitsamt seinen 15 kt liegen und der Sekundentakt schob ein **stehendes** Flugzeug weiter. Am Symbol stand dabei 0: dieselbe Maschine, zwei sich widersprechende Aussagen.
- **Kein Rückfall auf die VATSIM-Geschwindigkeit**, wenn der Sim 0 meldet. Eine 0 aus dem Sim heißt, dass das Flugzeug **steht**; der 29 s alte VATSIM-Wert sagt nur, dass es damals rollte. Ihn zu übernehmen ließe ein stehendes Flugzeug über die Karte wandern.
- **Kein Kursvergleich.** Der gemeldete Kurs ist so alt wie die Position — in der Platzrunde nach 29 s womöglich der exakte Gegenkurs. Ein Kursvergleich würde die richtige Zuordnung dann aktiv ablehnen.

**Bleibt bewusst so:** Ein Flugzeug, das **nur** VATSIM kennt, wird weiter fortgerechnet und rutscht deshalb noch, wenn es in Wirklichkeit schon steht. Es gibt keine Sim-Information, die das korrigieren könnte, und die 29 s alte Meldung ist die einzige vorhandene.
...
CREATE TABLE calendar_events (
    ...
    dtend     TEXT,
    location  TEXT,
    route     TEXT,        -- CSV aller ICAOs der Strecke (FriesenBummel)
...
-- Persistente FriesenBummel-Rennen (vom Poller beim Kalender-Sync oder Admin manuell angelegt)
CREATE TABLE bummel_races (
    ...
    manual_fields TEXT DEFAULT NULL,     -- #19: CSV der im Admin von Hand gesetzten Felder;
                                         --   der Kalender-Sync lässt genau diese stehen
    badge_name   TEXT DEFAULT NULL,      -- Kurzname für das Badge-PNG; NULL/leer = name.
                                         --   Der volle Event-Name sprengt die runde Grafik.
                                         --   Steht NICHT in manual_fields (kein Kalender-Pendant)
    created_at   TEXT NOT NULL
);
