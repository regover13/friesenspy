...
**Merker liegen im Cookie, nicht in `localStorage` (ab v13.6.0).** `_prefLies(key)` / `_prefSchreib(key, wert)` sind die einzige Ablage für alle Karten-Merker; darunter liegt **ein** Cookie `fs_karte` (JSON, `path=/`, `max-age` ein Jahr, `SameSite=Lax`). Die neun Schlüssel behalten ihre Namen (`friesenspy_layer`, `friesenspy_aip`, `friesenspy_platzrunden`, `friesenspy_fse_plaetze`, `friesenspy_fse_zonen`, `friesenspy_verkehr`, `fs_verkehr_schilder`, `friesenspy_trackup`, `friesenspy_movingmap`) und ihre Vorgaben.

*Warum der Umzug:* Im MSFS-Kniebrett überlebt `localStorage` einen Sim-Neustart nicht verlässlich. Aus 40 aufgezeichneten Panel-Starts kam eine vom Standard abweichende Basiskarte meist **nicht** zurück — und am 16.08.2026 um 16:28 kam sogar ein **veralteter** Wert zurück (`Light` von 14:59, obwohl um 15:21 `Satellit` gewählt war). Das ist der Beleg gegen einen Anwendungsfehler: Der Code liest den Schlüssel oder liest ihn nicht; einen früheren Stand kann nur die Speicherschicht selbst liefern. Die Sonde in der Selbstdiagnose meldet trotzdem `localStorage: true` — sie schreibt und liest im selben Atemzug und kann „funktioniert" von „überlebt einen Neustart" nicht unterscheiden. Cookies halten dort dagegen nachweislich: Das Login-Gate ist aktiv, und der erste Panel-Aufruf einer frischen Sitzung bekommt `200 OK` ohne Anmeldung — das Anmelde-Cookie war also schon da.

*Zweites Standbein:* `_prefSchreib` schreibt weiterhin nach `localStorage` mit (falls Cookies abgeschaltet sind), und `_prefLies` holt einen Schlüssel, den das Cookie nicht kennt, einmalig von dort und übernimmt ihn. Deshalb verliert beim Update niemand seine Einstellungen — und deshalb braucht die Übernahme **keine** Liste aller Schlüssel. Wichtig ist die Reihenfolge: Das Cookie hat Vorrang. Läse `localStorage` zuerst, gewänne genau der veraltete Wert wieder, gegen den die Umstellung gebaut ist (`test_cookie_schlaegt_veralteten_localstorage_wert`).
