...
**`consolidate_flights(conn, *, statsim_correct=True, shrink_margin_min=10)`** — läuft idempotent in `init_db()` **vor** dem Anlegen des partiellen Unique-Index (Reihenfolge zwingend, sonst Constraint-Verletzung) und ist als `scripts/consolidate_flights.py` (mit `--dry-run`) manuell aufrufbar. Reversibler Cleanup in vier Schritten: (A) mehrere offene Flüge je cid → frühesten behalten, Rest `superseded_by`; (B) exakte Duplikate (gleiche cid+logon_time) → offenen bzw. niedrigste id behalten, Rest `superseded_by`; (C) Zombie-Logoffs auf die gedeckelte letzte Position korrigieren (nur wenn das die Dauer um ≥ `shrink_margin_min` verkürzt); (D) StatSim-Backstop: grob unplausible FS-Dauern (> 2× StatSim + 10) auf den StatSim-Wert korrigieren. Rückgängig: `UPDATE flights SET superseded_by = NULL`.
