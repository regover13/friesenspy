...
| `aircraft_payloads` | Zuladung je Flugzeugtyp (Admin-editierbar): `mtow_kg`/`empty_kg`/`fuel_kg` (volle Tanks)/`crew_kg` (Pilot, Default 85, zählt nicht als Fracht) → `payload_kg = max(0, mtow−empty−fuel−crew)`, direkt überschreibbar; `source` ∈ `manual` \| `llm` \| `default`; `make_model` (Klartext von Claude) |
...
4. **Geo-Kontinuität** (`_segments_continuous`, nur mit `conn`): **Distanz-Budget** statt fester Radius — der Abstand zwischen der **letzten Position von Segment 1** und der **ersten Position von Segment 2** darf höchstens `Lückendauer × 600 kt + 10 nm` betragen. Das löst den Fall „Pilot 10 Min ohne Netz, Sim fliegt weiter" (Reconnect taucht stromabwärts auf), weist aber Teleports ab. **Abgeflogen-Regel**: endete Segment 1 bereits **gelandet am FP-Ziel** (letzte Position im Umkreis `_ARRIVED_RADIUS_KM` = 10 km **und** am Boden, `groundspeed ≤ _ARRIVED_MAX_GS_KT` = 40 kt), ist der Flugplan erfüllt — Segment 2 ist dann ein **neuer Flug**, kein Reconnect. Das verhindert, dass ein Rückflug mit stehengebliebenem (stale) Flugplan in den Hinflug gemergt wird: der Rückflug startet ja ebenfalls „am Ziel" und passierte deshalb die Richtungsprüfung (Live-Test 2026-07-01, FRS102). Zusätzlich **Richtungsprüfung**: Segment 2 darf nicht deutlich weiter vom Ziel entfernt sein als das Ende von Segment 1. Fallback Merge, wenn keine GPS-Daten vorhanden.
...
**`reconstruct_orphaned_flights(conn)`** — läuft in `init_db()` nach `consolidate_flights`: rekonstruiert flights-Einträge für **verwaiste GPS-Tracks** (A1-Schadensbild): ein StatSim-Flug (abgeschlossen, > 5 min, mit Strecke), der von keinem aktiven FriesenSpy-Flug gedeckt ist, dessen Fenster (StatSim-Zeiten ± `_RECONSTRUCT_MARGIN_MIN` = 10 min Taxi-Rand, gedeckelt auf die Nachbar-Sessions) aber nachweislich Flugbewegung (≥ 2 Positionen mit `groundspeed ≥ 40`) in `position_history` enthält, bekommt einen Eintrag aus den **echten Positionsdaten** (Dauer/Distanz/Block aus dem Track, Strecke/Callsign/Muster aus StatSim). Idempotent (einmal rekonstruiert = gedeckt); läuft nie für offene Sessions; Historie ohne Track bleibt unberührt.
