...
**Client-Polling & Rate-Limiting**: `/api/prefiles` und `/api/teamspeak` werden vom SSE-`positions`-Handler getrieben (einmal pro Poll-Zyklus). Die zusätzlichen 15-s-`setInterval` für beide feuern **nur als Fallback bei getrennter SSE** (`sseSource.readyState !== OPEN`) — sonst doppelte Last. `/api/live` läuft als unabhängiger 15-s-Fallback (iOS/PWA). nginx limitiert **nur `/api/`** (`zone friesenspy_limit`, `rate=120r/m`, `burst=30 nodelay`, `limit_req_status 429`); `/api/sse` ist ausgenommen (längster Prefix), SPA-Shell/statische Assets/PWA-Icons sind **nicht** limitiert, damit der Asset-Schwall beim Laden den Burst nicht sprengt. Historisch (`rate=30r/m, burst=10` für `location /`) verursachte das verbreitete 429 — vom Nutzer teils als „Fehler 409" gemeldet.
