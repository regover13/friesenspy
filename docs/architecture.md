...
| `calendar_events` | FriesenFlieger Google-Kalender (alle 6h synchronisiert, UID als Primary Key); `route` (CSV aller ICAOs) + `is_bummel` (Flag) für die FriesenFliegerBummel-Erkennung; `delete_stale_calendar_events` räumt bei jedem Sync Termine weg, die im Google-Kalender gelöscht wurden (Mark-and-Sweep über die im Lauf gelieferten UIDs, begrenzt auf das Sync-Fenster ±365/90 Tage) — `bummel_races`/`transport_events` bleiben davon unberührt |
...
**FriesenKutter-Fortschritt (`compute_transport_progress`).** Baut seit v8.0.0 wie der Bummel auf `canonicalize_legs` auf (GPS-Landung statt Disconnect/Refile). Ein Flug ist Feed-relevant, wenn Start UND Ziel (`departure`/`arrival` aus `canonicalize_legs`, bereits GPS-korrigiert — `gps_arrival` hat bewusst keinen Flugplan-Fallback, s. `_gps_flights_for_positions`) in der Streckenmenge liegen (dep≠arr). **Beladen** ist nur, wer am `destination` ankommt; Rückflüge zählen 0 kg, erscheinen aber im Feed. Beladene Flüge füllen das Fracht-Manifest nach Abflugzeit per **Co-Load**: jeder Flug verteilt seine Zuladung in Manifest-Reihenfolge über die noch nicht vollen Frachtarten, je Frachtart gekappt durch `per_flight_max_kg`; der Rest fließt in die nächste Frachtart (ein Flug kann mehrere Frachtarten tragen → `flights[].cargo_lines`). Zuladung je Typ aus `aircraft_payloads` (Fallback: `transport_default_payload_kg` aus `app_settings`) — sie ist der **Eingang** der Co-Load-Verteilung. **Durchgängig Netto (v8.8.0/#63):** die ausgewiesene Menge eines Flugs (`tonnage_kg`) ist die tatsächlich ins Manifest verteilte Summe, NICHT die Musterzuladung — Überschuss ohne Manifest-Platz (Kappung oder bereits volle Frachtarten) zählt nicht als geliefert, sodass `total_kg` == Σ `delivered` gilt und der Balken nie überzeichnet (vorher zählte `total_kg` die volle Musterzuladung, konnte also z. B. 85 % zeigen, obwohl nur 57 % ins Manifest passten). **Ausnahme:** ein Event OHNE Manifest (keine `cargo`-Zeilen) hat keine Frachtart zum Verteilen und bleibt ein schlichter Brutto-kg-Zähler (Musterzuladung). Der `cargo`-Response trägt je Frachtart `per_flight_max_kg` (fürs Kappungs-Badge in der Legende). Der Feed wird absteigend (neueste oben) zurückgegeben, mit gecachtem KI-Spruch je Flug (`transport_quips`) und `summary_quip`. Der Poller-Job `transport_event_check` latcht Start/Ziel/Feierabend, sendet je einmal einen Push, erzeugt (falls aktiviert) Flug-Sprüche im Hintergrund (`asyncio.to_thread`) und beim Feierabend die Tagesend-Zusammenfassung. **Der Feierabend-Latch wartet auf Nachzügler** (`transport_anyone_in_progress`, analog `_bummel_anyone_in_progress` beim Reveal): ist `dtend` erreicht, aber noch ein offener FRS-Flug mit Start auf der Strecke unterwegs (vor `dtend` begonnen, ohne Live-Ankunfts-Latch — ein Latch fixiert den Beitrag bereits), wird die Zusammenfassung verschoben, bis das Ergebnis final ist. Seit Live-Ankunft ohne Disconnect: die `loaded`-Bedingung ist zusätzlich wahr, wenn ein Eintrag in `transport_live_arrivals` existiert (`(cid, logon_time)` — gesetzt vom Poller via `check_live_arrival`, sobald ein noch offener Flug innerhalb `_BUMMEL_AIRPORT_RADIUS_KM` um `destination` auf `< _BLOCK_GS_KT` abbremst); dieser Latch hebt auch den Strecken-Filter auf, sodass die Fracht selbst dann gezählt bleibt, wenn der Pilot später weit außerhalb der Strecke disconnectet. Zusätzlich werden aktuell offene Flüge (`open_transport_flights`) mit Start auf der Strecke in den Feed aufgenommen (0 kg bis der Latch greift) — bisher wurden sie komplett ignoriert, da `canonicalize_flights` einen abgeschlossenen Flug voraussetzt. **Ober-Zeitgrenze (v8.8.7, Live-Fund 06.07.):** der Offen-Zweig überspringt Flüge, deren Verbindungs-Logon nach `end` (= min(now, dtend)) liegt — ohne diese Grenze zog jeder gerade eingeloggte Abflieger eines Streckenplatzes in JEDES Event mit demselben Startplatz, auch in längst beendete (Fund: ein laufender EDWG→EDWY-Flug erschien im abgelaufenen Test-Event EDWG→EDXP als „unterwegs", weil beide EDWG teilen). Bei laufendem Event ist `end == now`, die Grenze greift also nur bei beendeten Events (Nachzügler nach `dtend`).

**Reservierung, Teilnehmer, Verluste (v7.5.0).** Ergänzend zur Ankunfts-Zählung reserviert `compute_transport_progress` die Zuladung eines noch offenen Fluges Richtung Ziel bereits ab dem Einloggen am Streckenflugplatz (`in_air`, `reserved_kg`) in einem von `delivered` getrennten Topf (`reserved_alloc`/`reserved_total_kg`) — läuft nie rückwärts in den gelieferten Fortschritt und verschwindet mit dem Flug (Latch, Landung anderswo, Disconnect). **`airborne` (v8.8.5/#62-Folgefund):** `in_air` bedeutet nur „offene Reservierung Richtung Ziel", NICHT „abgehoben" — ein am Streckenplatz geparkter Pilot (gs 0, noch kein offenes GPS-Leg) reserviert bereits, ist aber erst „am Start". Das Flug-Dict trägt daher zusätzlich `airborne` (= `current_leg_by_cid` kennt ein offenes, abgehobenes Leg für diese cid); das Frontend zeigt „am Start 🅿️" bei `in_air && !airborne`, „unterwegs ✈️" erst mit `airborne`. Reservierung/Zählung sind davon unberührt (Live-Fund 06.07.: drei Piloten standen bei gs 0 am Vorfeld EDWG und erschienen fälschlich als „unterwegs"). Zusätzlich liefert die Funktion `participants` (Bummel-Analogie: eine Zeile pro Pilot mit `delivered_kg`/`reserved_kg`/`lost_kg` und Status `flying`\|`arrived`\|`returning`\|`done`). **Rückflug-Status: Leg-genaue Erkennung (v8.8.3/#65, #66).** Eine offene Verbindung, deren aktuell LAUFENDES Leg am `destination` startet, gilt als Rückflug. Ursprünglich (v8.8.2) wurde dafür `_first_pos` auf die ERSTE Position der GESAMTEN Verbindung angewandt — das brach, sobald eine durchgehend offene Verbindung mehrere Legs nacheinander enthielt (Rückflug landet, danach startet ein neuer, unabhängiger Flug in DERSELBEN Verbindung ohne Disconnect): die Referenzposition blieb für die ganze Verbindungslebensdauer eingefroren, sodass der neue Flug fälschlich weiter als „Rückflug" erschien (Live-Fund 06.07., EDWG-EDXP-Test). **Fix:** `current_leg_by_cid` löst je Pilot das von `canonicalize_legs` bereits korrekt per GPS erkannte, aktuell offene (noch nicht gelandete) Leg auf (eigener `canonicalize_legs`-Aufruf mit `end=now`, ungekappt durch `dtend`) und liefert dessen EIGENEN, richtigen `departure`-Wert als `dep`-Referenz — Fallback auf die alte Heuristik nur, wenn kein GPS-Leg erkannt wurde (z. B. trackless). Zusätzlich musste der „schon geliefert → Verbindung überspringen"-Schutz (`loaded_conn_logons`, #23 Review C1) geschärft werden: er darf nur greifen, solange seit der Lieferung noch KEIN neues Leg begonnen hat (`int(cid) not in current_leg_by_cid`) — sonst blendet er die komplette restliche Lebensdauer der Verbindung aus, sobald irgendwann einmal geliefert wurde, selbst wenn seither Rückflug UND ein weiterer, neuer Flug passiert sind. `_returning_pilot_landed(conn, cid)` prüft ergänzend die AKTUELLE Live-Position (`live_positions`, dieselbe Quelle wie `check_live_arrival`): ist sie am Boden (`groundspeed < _BLOCK_GS_KT`) — EGAL wo, die Position spielt für die Klassifikation keine Rolle mehr —, gilt der Rückflug als abgeschlossen, der Teilnehmer verschwindet aus Feed/`participants`. Verluste stehen in der neuen Tabelle `transport_cargo_losses` (Latch je `event_id+cid+logon_time`); `detect_transport_losses(conn, event)` klassifiziert einen abgeschlossenen Flug, der Richtung Ziel gestartet, aber nie dort angekommen ist, per letzter Position als `returned` (am Boden am Abflugplatz), `stolen` (am Boden anderswo) oder `sunk` (in der Luft verschwunden); der Poller-Job `transport_event_check` (60 s) ruft sie idempotent auf. **Zeitgrenze:** `detect_transport_losses` wertet dabei nur Flüge im Event-Fenster (`logon_time` bis `dtend`) und läuft nur bis zum Feierabend-Latch (`summarized_at`) — danach ist die Wertung final, ein Aufruf für ein bereits zusammengefasstes Event würde nur unnötig Flüge/Positionen laden (Final-Review-Fix). Der Anwesenheitsradius ist wie beim Bummel der feste, globale `_BUMMEL_AIRPORT_RADIUS_KM` = 4 km (#23) — `transport_events.radius_km` bleibt als Legacy-DB-Spalte bestehen (durchgereicht, s. Signatur), ist aber über die Admin-Endpoints nicht mehr setzbar und hat keine Wirkung mehr; einen per-Event-Radius (früher u. a. für kurze Strecken wie Wangerooge↔Harle) gibt es seit GPS-only Phase 2 nicht mehr. **Verlust-Override (v7.5.3):** Ein GPS-belegter Verlust (`stolen`/`sunk`) überstimmt eine Lieferung, die NUR am Flugplan-Text hängt (kein Live-Latch, keine GPS-Ankunft am Ziel — interne Markierung `_fp_only` in `compute_transport_progress`); ohne diese Regel könnte praktisch nie jemand versinken oder beklaut werden, weil Kutter-Piloten normalerweise einen Plan zum Ziel aufgeben (Demo-Befund 02.07.). Echte Latch-/GPS-Lieferungen bleiben unantastbar, und `detect_transport_losses` wertet Flüge **ohne jeden GPS-Track** (z. B. StatSim-rekonstruiert) grundsätzlich nicht (keine Position = keine Aussage — sonst würde der Override echte StatSim-Lieferungen kippen). Verlust-/Rückbringer-Zeilen zeigen ihre Bordladung als Co-Load-Aufschlüsselung (`cargo_lines`: Zuladung in Manifest-Reihenfolge, gekappt durch pro-Flug-Obergrenze UND Frachtart-Ziel — die Musterzuladung ist nur der Verteilungs-Eingang; reine Anzeige, ändert `delivered`/offen nicht). **`lost_kg` ist Netto (v8.8.8, Fix Live 06.07.):** die verlorene Menge = Σ der tatsächlich zugeordneten Frachtart-Anteile (= `cargo_lines`), NICHT die volle Musterzuladung. Überschuss-Kapazität, die keiner Manifest-Frachtart entspricht (Pro-Flug-Kappung oder nur wenige Frachtarten), war nie Event-Fracht und zählt nicht als verloren — konsistent mit `reserved_kg`/`tonnage_kg` (durchgängig Netto, #63). Fund: PZ04 mit 290 kg Musterzuladung, aber nur 120+40=160 kg Event-Fracht an Bord → früher fälschlich „290 kg verloren", jetzt korrekt 160. Bei Event OHNE Manifest bleibt die Musterzuladung als schlichter kg-Zähler stehen; `returned` verliert nichts (`lost_kg=0`, zeigt aber seine Bordladung). **Durchgängig Netto (v8.8.0/#63):** unterwegs befindliche (`in_air`) Flüge dagegen zeigen `reserved_kg` + `cargo_lines` **restkapazitäts-begrenzt** (nur was noch ins Ziel passt) — die volle Musterzuladung bleibt separat als `onboard_reserved_kg` erhalten (für die „belegt / an Bord"-Anzeige). So ist `Σ reserved_kg` der Feed konsistent mit `reserved_total_kg`. Beladene Flüge tragen analog `tonnage_kg` (Netto-Gutschrift) + `onboard_kg` (Musterzuladung). **UI-Aufteilung (v7.6.0, Nutzer-Entscheidung):** Der Live-Tab zeigt bei laufendem Event einen Kutter-Block (Analogie Bummel-Banner, 25-s-Poll): Balken + Mengen + „wer ist gerade mit welcher Ladung unterwegs" (Callsign statt Name, 🗺️-Sprung zur Karte via `switchToMapAndCenter`, Teilen + Zum-Kutter-Link) — bewusst OHNE Pro-Pilot-Bilanz; die Events-Ansicht zeigt Balken + Mengen + Flug-Feed, die frühere Teilnehmer-Summenliste ist dort entfallen (Bilanz ergibt sich aus dem Feed).
