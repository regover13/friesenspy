...
- **`render_kutter_badge(d: dict)`** — Kutter-Abschluss-Badge „Voll beladen!" (navy Kern wie `render_medal`) mit Callsign, Name, Flugzeugmuster, gelieferten kg, Event-Name + Datum. Hat ein Teilnehmer Fracht verloren, ergänzt ein Verlust-Abschnitt einen Titel (`_kutter_loss_label(stolen_kg, sunk_kg)`: **SPITZBOOV!** nur geklaut, **BADEMESTER!** nur versenkt, **SEEROVER!** beides, `None` bei keinem Verlust — pure Funktion, direkt testbar) plus eine Mengen-Zeile (z. B. „150 kg geklaut, 292 kg versenkt", ASCII-Komma).

Alle Badges tragen **Event-Name und Datum**. Der Abstand zum Schnitt (Bummel) wird signiert + sekundengenau über `_fmt_signed_delta(sec)` formatiert (z. B. „+1:23 zum Schnitt", bei `0` „punktgenau", `None` → leer). `_badge_entry_data` (`app/main.py`) liefert dafür zusätzlich `event` (Renn-Name aus `race.name`) und `delta_sec`; `_kutter_badge_data` (`app/main.py`) liefert analog `event` (Event-Name), `date`, `delivered_kg`, `stolen_kg`, `sunk_kg` aus `compute_transport_progress` (`participants`/`losses`).
