...
| `custom_airports` | Ergänzungs-Flugplätze (v8.5.0, #50): Plätze, die in `airportsdata` fehlen (z. B. Segelfluggelände ohne offizielle ICAO-Kennung — `icao` erlaubt dann einen Platzhalter-Code wie `ZZSALZ`, kein echter 4-Buchstaben-ICAO nötig). `lat`/`lon` Pflicht, `elevation_ft` optional (`NULL` macht die GPS-Rettung am Track-Ende, #53, konservativ; den Spawn-Guard, #49, bleibt sie permissiv). Admin-CRUD unter `/api/admin/airports`; `app/geo.py` konsultiert die Tabelle als zweite Quelle NACH `airportsdata` (Push-Modell über `geo.set_custom_airports()`, kein direkter DB-Zugriff in `geo.py`). Seed (idempotent, nur bei leerer Tabelle) enthält sechs in einer Live-Analyse (2026-07-05) bestätigte Plätze. Ein Admin-Write lehnt Codes ab, die bereits in `airportsdata` bekannt sind (`geo.is_known_in_airportsdata`), und stößt einen vollen `rebuild_flight_cache` an (der inkrementelle 7-Tage-Refresh würde ältere, durch den neuen Platz betroffene Flüge nicht heilen). |
...
- `GET /api/admin/airports` + `POST /api/admin/airports` + `DELETE /api/admin/airports/{icao}` (v8.5.0, #50) — CRUD für `custom_airports` (`list_custom_airports`/`upsert_custom_airport`/`delete_custom_airport`). `POST` lehnt Codes ab, die bereits in `airportsdata` bekannt sind (`geo.is_known_in_airportsdata`, `400`); Pflichtfelder `icao`/`lat`/`lon`, `elevation_ft` optional. Jeder erfolgreiche Write ruft `geo.set_custom_airports(...)` (Cache-Invalidierung — sofort wirksam, kein Neustart nötig) UND `rebuild_flight_cache(conn, full=True)` (der inkrementelle 7-Tage-Refresh würde ältere, durch den neuen/geänderten Platz betroffene Flüge nicht heilen).
