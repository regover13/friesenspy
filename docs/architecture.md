...
- **Im Kniebrett zeigt das Vollbild nur die Karte** (seit 02.09.2026): `toggleMapFullscreen` setzt über `_vollbildKlassePflegen()` die Klasse `karte-vollbild` am `<html>` — nicht am Karten-Wrapper, weil die Zurück-/Tab-Leiste bewusst außerhalb von `#app` am `</body>` hängt und über die Karte gar nicht ansprechbar wäre. Die Leiste klebte am oberen Rand und verdeckte alles, was die Karte dort anbietet, zuletzt die Eingabezeile der ICAO-Suche. Der Ausgang wandert dafür zurück in die Karte (`.map-fullscreen-btn`, unten links, `z-index: 1200` über der Herkunftsangabe); die Windanzeige weicht nach oben rechts aus. **Die Bedingung prüft zwei Dinge:** Vollbild an *und* kein Fenster offen — ein Modal liegt mit `z-index: 10000` über der Karte (3000) und damit über dem Ausgang, ohne die Leiste säße man fest. Deshalb ruft auch `_updatePanelBackBtn` die Pflege mit, das ohnehin nach jeder Eingabe und im Sekundentakt prüft.
