...
- **Türkise Flugzeuge werden sekundengenau gezeigt.** Ihre Position kommt dann nicht von VATSIM, sondern jede Sekunde direkt aus einem Simulator — auf den Meter statt bis zu 35 Sekunden alt wie bei VATSIM. Diese 35 Sekunden sind die Summe dreier Wartezeiten: 2–5 Sekunden, bis die Position überhaupt im VATSIM-Datenstand steht (gemessen an 1280 Piloten: Mittelwert 2,6 s, 99 % unter 5 s), bis zu 15 Sekunden, bis wir diesen Stand abrufen, und bis zu 15 Sekunden, bis die Karte ihn von uns holt. Höhe und Geschwindigkeit am Symbol stammen aus derselben Quelle, und im Popup steht die Quelle als eigene Zeile. Meist ist das seine eigene **FriesenBrügge**. Es kann aber auch das **Kniebrett eines anderen Piloten** sein: Wer im Simulator das Kniebrett offen hat, sieht die Flugzeuge um sich herum ebenfalls sekundengenau und kann sie melden — dann wird ein Pilot in Echtzeit gezeigt, ohne selbst etwas installiert zu haben. **Das ist standardmäßig abgeschaltet** und wird nur eingeschaltet, wenn die Gruppe es so will; im Popup steht dann „Kniebrett eines anderen Piloten". Hört die Quelle auf zu melden (Simulator geschlossen, Netz weg, Flugzeug außer Reichweite), wird das Flugzeug nach zehn Sekunden wieder blau und läuft normal auf VATSIM weiter
- **Im Kniebrett kommt Türkis von deinem eigenen Simulator.** Dort braucht es keine fremde Meldung: Der Simulator zeigt dir die Flugzeuge um dich herum ohnehin, und sobald das Kniebrett eines davon einem Piloten zuordnen kann, wird es türkis. Das ist die genaueste Anzeige, die es in FriesenSpy gibt — gemessen statt geschätzt, ohne einen einzigen Netzweg dazwischen. **Auch der Fremdverkehr zeigt es an**, dort aber anders: Sein Symbol bleibt dunkel, nur sein Saum wird türkis. Die Fläche eines Flugzeugs sagt weiterhin, ob es ein Friese ist — daran ändert die Quelle nichts. **Dein eigenes Flugzeug ist im Kniebrett immer türkis** — es braucht dafür keine Zuordnung, seine Position kommt ohnehin direkt aus dem Simulator, und genauer kennt die Karte kein Flugzeug. Verliert der Simulator ein Flugzeug aus den Augen, fällt es wieder auf VATSIM zurück
...
- **✈️ Verkehr** (Haken in der Ebenen-Auswahl) — zeigt zusätzlich **anderen Verkehr** im sichtbaren Kartenausschnitt, kleiner und dunkel, damit die Friesen die auffälligen bleiben. Die fremden Flugzeuge gleiten genauso wie die eigenen. Der Haken schaltet **nur** den Fremdverkehr; die Friesen sind immer da. Weit herausgezoomt bleibt die Ebene still — dort wären es hunderte Symbole ohne Aussage
  ...
  - Im **Kniebrett** (MSFS-Tablet, ab Paket **1.7.0**) kommen **beide Quellen zusammen**: Wo der Simulator ein Flugzeug kennt, liefert er dessen Bewegung — jede Sekunde gemessen statt zwischen zwei VATSIM-Meldungen geschätzt. Rufzeichen, Muster und Flugplan steuert VATSIM bei; der Simulator kennt sie nicht. Was nur VATSIM kennt (vPilot spawnt nicht jede Maschine), bleibt trotzdem stehen, und was nur der Simulator kennt, erscheint ohne Rufzeichen. Das gilt auch ohne VATSIM-Verbindung — dann siehst du den Verkehr, den der Simulator selbst erzeugt
  - **Welches Symbol zu welchem Rufzeichen gehört, wird einmal erkannt und dann behalten** — nicht in jeder Sekunde neu gesucht. Erkannt wird nur, wenn es eindeutig ist: Kommen zwei Flugzeuge in Frage, bleibt das Symbol lieber ohne Namen, denn ein falscher Name wäre schlimmer als gar keiner. Bleibt am Ende genau eines übrig, ist auch das eindeutig — fliegen fünf zusammen und vier sind erkannt, ist der fünfte klar
  - Ein Flugzeug, das **nur VATSIM** kennt, bewegt sich weiterhin geschätzt und rutscht deshalb noch ein Stück, wenn es in Wirklichkeit schon steht. Der Simulator weiß nichts über es, und die Meldung ist bis zu 35 Sekunden alt (gemessen, s. [Karte-Tab](#️-karte)) — mehr Information gibt es nicht
  - **Am Symbol steht immer alles**: Rufzeichen, Muster, Höhe und Geschwindigkeit — bei jedem Flugzeug, auch beim eigenen und auch bei Linienverkehr weit oben. Geparkte Maschinen sind ganz normal dabei
  - **Radar Label** (zweiter Haken direkt unter „Verkehr", erscheint nur bei eingeschalteter Ebene) — blendet die Beschriftungen am fremden Verkehr aus, wenn das Bild ruhiger sein soll. Über dem Platz stehen sonst schnell dreißig nebeneinander. Die Friesen und das eigene Flugzeug behalten ihres, und ein Klick aufs Symbol zeigt ohnehin alles
...
- **🔍 ICAO-Suche** (Lupe oben rechts) — einen Flugplatz nach seiner Kennung anspringen
- **Deckkraft-Regler** (oben links unter den Zoomknöpfen) — nur da, solange ein Kartenblatt liegt: regelt, wie stark die Sichtflug- bzw. Flugplatzkarte die Karte darunter verdeckt
...
**Die Karten-Legende unter der Karte erklärt all das noch einmal an Ort und Stelle** — mit denselben Symbolen, die auf der Karte stehen. Sie ist in zwei Abschnitte geteilt: *Flugzeug-Marker* (was Blau, Grau und Türkis bedeuten und wie alt eine VATSIM-Position ist) und *Bedienelemente* (jede Karte und jede Zusatzebene mit einem eigenen Satz, dazu Kompass, Moving Map, ICAO-Suche und der Deckkraft-Regler). Im Kniebrett steht an der Stelle der Türkis-Zeile eine eigene: Die der Webseite verweist auf die Download-Seite der FriesenBrügge, und ein Link ließe sich im Tablet ohnehin nicht öffnen — vor allem aber gilt dort eine andere Erklärung, weil Türkis im Kniebrett aus dem eigenen Simulator kommt und nicht aus einer fremden Meldung.
...
Nach dem Renn-Ende steht in der Statuszeile, **woran der Abschluss gerade hängt**: entweder „Abschluss wartet auf FRS…" mit den Callsigns derer, die noch fliegen, oder „alle am Boden — Abschluss wartet bis Logout oder hh:mm:ssz zur Erfassung der Blockzeiten". Die Uhrzeit ist ein *spätestens*: Sie verschiebt sich nach hinten, wenn jemand doch noch einmal rollt, und entfällt, sobald sich der Letzte ausloggt. Einmal enthüllt, bleibt das Ergebnis dauerhaft sichtbar. **Seit v8.10.0** wird die Wertung eines enthüllten Rennens beim ersten Abruf eingefroren (Snapshot) — das lädt die Übersicht spürbar schneller; eine Korrektur greift erst, wenn das Rennen im Admin erneut gespeichert wird. Übersichten zeigen zudem nur die letzten 12 Monate (ältere Rennen werden ausgeblendet, nicht gelöscht).
...
- **Eigener Name auf dem Badge:** Auf der runden Scheibe ist wenig Platz — ein Event wie „Montagsflüge in Deutschland — Aach-Bummel" schrumpft dort zur Unleserlichkeit. Im Admin steht deshalb bei jedem Rennen ein Feld **„Name auf dem Badge"**; trägst du dort „Aach-Bummel" ein, steht nur das auf der Grafik. Lässt du es leer, nimmt das Badge weiter den vollen Event-Namen (er steht als grauer Vorschlag im Feld). Überall sonst — Events-Liste, Ergebnistext fürs Forum, Benachrichtigungen — bleibt der volle Name stehen. Änderst du den Kurznamen nachträglich, holt sich das Forum von selbst die neue Grafik.
...
- **Positionen melden** — ob ein offenes Kniebrett zurückmeldet, welche Flugzeuge es erkannt hat. Drei Stufen: *aus*, *nur die eigene Position*, *alle erkannten Friesen* (der Simulator sieht sie bis rund 90 km weit). Einstellbar global und für einzelne Piloten; der globale Wert ist dabei die Obergrenze. **Vorgabe ist „aus".** Die Umstellung wirkt binnen zehn Sekunden, ohne dass jemand etwas neu installieren muss. Daneben steht, wer gerade wen versorgt — auf der Karte sieht man sonst nur den Punkt, nicht seine Quelle.
...
gibt). Die Endpunkte stehen in [`docs/api.md`](docs/api.md).
...
│   ├── database.py    # SQLite WAL, alle DB-Funktionen (inkl. canonicalize_legs, flight_cache, app_settings, Pilots-CRUD, airport_links = AIP-VFR-Karten-Links)
