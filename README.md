...
- **Fracht direkt im Kalendertermin:** Eine Zeile wie `Fracht EDWG: 1000 Krabbenbrötchen, 500 Friesentee` in der Termin-Beschreibung befüllt das Manifest beim Kalender-Sync automatisch (Namen gegen den Katalog abgeglichen). Der Marker braucht **genau einen Startplatz** (das ICAO direkt hinter `Fracht`); je Ware an einem anderen Platz eine eigene Zeile: `Fracht EDDW: 300 Birnen`. Eine `Fracht:`-Zeile **ohne** ICAO — oder mit mehreren — wird abgelehnt (jede Frachtart braucht genau ihren einen Startplatz). Nur beim erstmaligen Anlegen — spätere Admin-Bearbeitungen bleiben bei erneutem Sync erhalten.
