...
- **Anlegen:** **ausschließlich manuell im Admin** (Entscheidung 20.07.2026). Der Kutter-Import aus dem Kalender ist **deaktiviert** — ein Kalendertermin mit „Kutter"/„FriesenKutter" im Titel oder Text wird bewusst **nicht** nach FriesenSpy übernommen (weder als Event noch als Objekt), damit es kein Doppel im Events-Tab und kein Matching zwischen manuellem und Kalender-Objekt gibt. Solche Kalendertermine dienen nur der Ankündigung (Discord/Forum-Link). Andere Kalender-Events und der Bummel bleiben unberührt (Anzeige + Erinnerung).
